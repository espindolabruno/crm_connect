...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await register(accountName, userName, email, password);
    } catch (err: any) {
      setError(err.message || 'Falha ao registrar agência. Tente novamente.');
    } finally {
      ...
      <div className="w-full max-w-md space-y-8 relative z-10">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-slate-400">
            Crie sua conta e configure seu funil inteligente
          </p>
        </div>
        ...
        <div className="mt-8 bg-slate-900/60 backdrop-blur-xl border border-slate-800 p-8 rounded-2xl shadow-2xl">
          <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            <div>
              <label htmlFor="accountName" className="block text-sm font-medium text-slate-300">
